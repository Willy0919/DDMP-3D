...
def im_detect_3d(im, depth, net, rpn_conf, preprocess, p2, gpu=0, synced=False):
    ...
    # profile(net, inputs=(im,depth, ))
    #print('======================++++++++',flops/1e9,params/1e6)
    cls, prob, bbox_2d, bbox_3d, feat_size, rois, prop_feats = net(im, depth)
    print('===========prop+feat,', prop_feats.shape)
    ...
    return aboxes,prop_feats, scale_factor
...
def test_kitti_3d(dataset_test, test_split, net, rpn_conf, results_path, test_path, use_log=True):
    ...
    preprocess = Preprocess([rpn_conf.test_scale,1760], rpn_conf.image_means, rpn_conf.image_stds, rpn_conf.depth_mean,
    ...
    for imind, impath in enumerate(imlist):
        ...
        if rpn_conf.depth_channel == 3:
            ...
            dir_path,file_path = os.path.split(impath)
            depthpath = os.path.join(dir_path.replace('image_2', 'depth_2'),str(int(file_path.split('.')[0]))+'.pkl')
            with open(depthpath, 'rb') as file:
                depth = pickle.load(file)
        else:
            depth = cv2.imread(impath.replace('image_2', 'depth_2'), cv2.IMREAD_UNCHANGED)
            print('=========>',impath.replace('image_2', 'depth_2'))
