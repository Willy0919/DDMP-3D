...
class RPN_3D_loss(nn.Module):
    ...
    def forward(self, cls, prob, bbox_2d, bbox_3d, bbox_3d_z_dep, imobjs, feat_size, bbox_vertices=None, corners_3d=None):
        ...
        bbox_x3d_dep = bbox_3d_z_dep[:,:, 0]
        bbox_y3d_dep = bbox_3d_z_dep[:,:, 1]
        bbox_z3d_dep = bbox_3d_z_dep[:,:, 2]
        bbox_w3d = bbox_3d[:, :, 3]
        bbox_h3d = bbox_3d[:, :, 4]
        bbox_l3d = bbox_3d[:, :, 5]
        bbox_ry3d = bbox_3d[:, :, 6]
        ...
        coords_abs_x_dep = torch.zeros(cls.shape[0:2])
        coords_abs_y_dep = torch.zeros(cls.shape[0:2])
        coords_abs_z_dep = torch.zeros(cls.shape[0:2])
        ...
        bbox_x3d_dn_dep = bbox_x3d_dep * self.bbox_stds[:, 6][0] + self.bbox_means[:, 6][0]
        bbox_y3d_dn_dep = bbox_y3d_dep * self.bbox_stds[:, 6][0] + self.bbox_means[:, 6][0]
        bbox_z3d_dn_dep = bbox_z3d_dep * self.bbox_stds[:, 6][0] + self.bbox_means[:, 6][0]
        ...
        bbox_z3d_dn_dep = src_anchors[:, 4].unsqueeze(0) + bbox_z3d_dn_dep
        bbox_x3d_dn_dep = src_anchors[:, 4].unsqueeze(0) + bbox_x3d_dn_dep
        bbox_y3d_dn_dep = src_anchors[:, 4].unsqueeze(0) + bbox_y3d_dn_dep
        ...
        for bind in range(0, batch_size):
            ...
            if gts_val.shape[0] > 0 or gts_ign.shape[0] > 0:
                ...
                if self.use_hill_loss:
                    ...
                    hill_z3d_dep = bbox_z3d_dn_dep[bind].unsqueeze(0)
                ...
                if fg_num > 0:
                    ...
                    bbox_x3d_dn_dep_fg = bbox_x3d_dn_dep[bind, fg_inds]
                    bbox_y3d_dn_dep_fg = bbox_y3d_dn_dep[bind, fg_inds]

                    bbox_z3d_dn_dep_fg = bbox_z3d_dn_dep[bind, fg_inds]
                    ...
                    bbox_x3d_dn_dep_fg /= imobj['scale_factor']
                    bbox_y3d_dn_dep_fg /= imobj['scale_factor']
                    ...
                    # coords_2d_dep = torch.cat((bbox_x3d_dn_dep_fg[np.newaxis, :] * bbox_z3d_dn_dep_fg[np.newaxis, :],
                    #                        bbox_y3d_dn_dep_fg[np.newaxis, :] * bbox_z3d_dn_dep_fg[np.newaxis, :],
                    #                        bbox_z3d_dn_dep_fg[np.newaxis, :]), dim=0)
                    # coords_2d_dep = torch.cat((coords_2d_dep, torch.ones([1, coords_2d_dep.shape[1]])), dim=0)
                    #
                    # coords_3d_dep = torch.mm(p2_inv, coords_2d_dep)
                    ...
                    coords_abs_z_dep[bind, fg_inds] = torch.abs(bbox_z3d_dn_tar - bbox_z3d_dn_dep_fg)
        ...
        if np.sum(bbox_weights) > 0:
            ...
            if self.bbox_3d_lambda:
                ...
                bbox_x3d_dep = bbox_x3d_dep[:, :].view(-1)
                bbox_y3d_dep = bbox_y3d_dep[:, :].view(-1)
                bbox_z3d_dep = bbox_z3d_dep[:, :].view(-1)
                ...
                loss_bbox_x3d_dep = F.smooth_l1_loss(bbox_x3d_dep[active], bbox_x3d_tar[active], reduction='none')
                loss_bbox_y3d_dep = F.smooth_l1_loss(bbox_y3d_dep[active], bbox_y3d_tar[active], reduction='none')
                loss_bbox_z3d_dep = F.smooth_l1_loss(bbox_z3d_dep[active], bbox_z3d_tar[active], reduction='none')
                ...
                loss_bbox_x3d_dep = (loss_bbox_x3d_dep * bbox_weights[active]).mean()
                loss_bbox_y3d_dep = (loss_bbox_y3d_dep * bbox_weights[active]).mean()
                loss_bbox_z3d_dep = (loss_bbox_z3d_dep * bbox_weights[active]).mean()
                ...
                bbox_3d_loss = (loss_bbox_x3d + loss_bbox_y3d + loss_bbox_z3d + loss_bbox_z3d_dep+loss_bbox_x3d_dep+loss_bbox_y3d_dep)
