...
class DynamicWeightsCat33(nn.Module):
    ...
    def __init__(self, channels, rgb_inchannel, group=1, kernel=3, dilation=(1, 1, 1), shuffle=False, deform=None):
        ...
        if deform == 'deformatt':
            ...
            ## image offset, depth kernel

            self.cata_off = nn.Conv2d(in_channel, 18, 3, padding=dilation[0], dilation=dilation[0], bias=False)
            self.catb_off = nn.Conv2d(in_channel, 18, 3, padding=dilation[1], dilation=dilation[1], bias=False)
            self.catc_off = nn.Conv2d(in_channel, 18, 3, padding=dilation[2], dilation=dilation[2], bias=False)
            ...
            self.cata = nn.Conv2d(in_channel, group * kernel * kernel, 3, padding=dilation[0], dilation=dilation[0],
                                  bias=False)
            self.catb = nn.Conv2d(in_channel, group * kernel * kernel, 3, padding=dilation[1], dilation=dilation[1],
                                  bias=False)
            self.catc = nn.Conv2d(in_channel, group * kernel * kernel, 3, padding=dilation[2], dilation=dilation[2],
                                  bias=False)
    ...
    def forward(self, rgb_feat, depth1, depth2, depth3):
        ...
        N, C, H, W = x.size()
        R = C // self.group
        dynamic_filter1, att1 = self.filter1(depth1)
        dynamic_filter2, att2 = self.filter2(depth2)
        dynamic_filter3, att3 = self.filter3(depth3)
        if self.deform == 'deformatt':
            offset_att1 = self.cata_off(x)
            offset_att2 = self.catb_off(x)
            offset_att3 = self.catc_off(x)

            offset1 = offset_att1[:, :18, :, :]
            att1 = att1.view(N, -1, H * W).view(N, 1, -1, H * W)  # N, 1, 9, H*W
            att1 = att1.sigmoid()

            offset2 = offset_att2[:, :18, :, :]
            att2 = att2.view(N, -1, H * W).view(N, 1, -1, H * W)  # N, 1, 9, H*W
            att2 = att2.sigmoid()

            offset3 = offset_att3[:, : 18, :, :]
        ...
        if self.deform == 'deform':
            ...
            offset1 = self.cata_off(x)
            offset2 = self.catb_off(x)
            offset3 = self.catc_off(x)
        ...
        ## N, K, H, W --> N*H*W
        dynamic_filter1 = self.softmax(dynamic_filter1.view(N, self.group, self.K, -1).permute(0, 1, 3, 2).contiguous().view(-1,
                                                                                                                self.K))
        dynamic_filter2 = self.softmax(dynamic_filter2.view(N, self.group, self.K, -1).permute(0, 1, 3, 2).contiguous().view(-1,
                                                                                                                self.K))
        dynamic_filter3 = self.softmax(dynamic_filter3.view(N, self.group, self.K, -1).permute(0, 1, 3, 2).contiguous().view(-1,
                                                                                                                self.K))
...
class Warp(nn.Module):
    def __init__(self, channels, dilation=1, kernel=3, groups=1,groups_k=1, deform=False, normalize=None, att=False,
        ...
        self.conv_1 = nn.Conv2d(channels, kernel * kernel*groups_k, kernel_size=3, padding=dilation, dilation=dilation,
                                bias=False)
    ...
    def forward(self, x):
        ...
        R = C // self.group
        offset_aff = self.off_conv(x)
        offset = offset_aff[:,:18,:,:]
        aff = offset_aff[:,-9:,:,:]
